...
const Features = () => {
  return (
    <div className="flex items-start justify-start flex-col  desktop:h-[610px] h-auto  py-5" id="features">
      ...
      <div className="grid desktop:grid-cols-4 desktop:grid-rows-1 grid-rows-4 grid-cols-1  mx-auto   flex-1 w-screen desktop:divide-x-[2px] divide-y-[2px] desktop:divide-y-0 divide-black-primary desktop:w-auto">
        {features.map((feature, index) => (
          ...
          >
            <h3 className="desktop:text-title-primary sm:text-title-secondary text-scroll-text desktop:font-semibold font-medium italic  pb-5 ">
              {feature.title}
            </h3>
            <p className="desktop:text-paragraph-primary sm:text-paragraph-secondary text-paragraph-mobile ">
              {feature.description}
            </p>
          </div>
        ))}
